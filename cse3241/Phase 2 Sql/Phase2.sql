...
GO
-- Create Triggers
-- This trigger will add a row into a audit table when it is suspected that 
-- someone has more than one membership. This help to enforce the constraint that
...
begin
	...
	begin
		...
		values
		(@mID, 'After Insert: This member seems like already exists.');
...
begin
	...
	begin
		insert into MEMBERS_AUDIT(id, comments)
		values
		(@mID, 'After Update: This member seems like already exists.');
		PRINT 'After Update: A Duplicate member being added';
	end
end
go

-- This trigger enforce the constraint that only employees working in store 
-- can have shifts.
...
begin
	...
	begin
		...
		COMMIT;
		PRINT 'Works Instead: Work on shift record added.'
	end
end
go
